...
{
    public partial class FormAlbumAdmin : Form
    {
        private CN_GetData ObjectCN = new CN_GetData();
        Boolean isInsert = true;
        int id_album = 0;

        public FormAlbumAdmin()
        {
            InitializeComponent();
            GetComboCandidata();
        }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            try
            {
                if (isInsert == true)
                {
                    ObjectCN.InsertarAlbum(cmbId_Can.Text, txtNomAl.Text, cmbEst.Text, dtpFechaRe.Value);
        ...
        private void FormAlbumAdmin_Load(object sender, EventArgs e)
        {
            CargarDatosAlbum();
        }
